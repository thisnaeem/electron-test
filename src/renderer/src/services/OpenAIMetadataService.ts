...
export class OpenAIMetadataService {
...
Respond with only the title, description, and keywords in the specified format.`
      ...
      console.log(`📸 Processing ${filename} with OpenAI ${this.selectedModel}`)
      ...
      if (!title || !description || keywords.length === 0) {
        throw new Error('Failed to parse OpenAI response - missing required fields')
      }
      ...
      console.log(`✅ OpenAI metadata generated for ${filename}:`, {
      ...
      console.error(`❌ OpenAI metadata generation failed for ${filename}:`, error)
      return {
        filename,
        title: '',
        description: '',
